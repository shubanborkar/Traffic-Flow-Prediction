...
def evaluate_logistic_model(log_model, X_test, y_test, threshold):
    ...
    auc = roc_auc_score(y_test_binary_first, y_pred_prob)
    return accuracy, auc, y_pred, y_test_binary_first
...
def plot_predictions(predictions, actuals, model_name, n_samples=3):
    plt.figure(figsize=(12, 4))
    for i in range(min(n_samples, predictions.shape[0])):
        plt.subplot(n_samples, 1, i+1)
        sns.lineplot(data=pd.DataFrame({'Actual': actuals[i, :, 0][:5], 'Predicted': predictions[i, :, 0][:5]}), palette='deep')
        if i == 0:
            plt.legend()
...
def main(dataset_path, dataset_type='metr_la', load_saved_models=False):
    ...
    # 8. Train and evaluate Logistic Regression model (skip plotting due to shape mismatch)
